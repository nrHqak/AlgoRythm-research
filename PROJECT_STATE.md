...
- **Claim boundaries** — `pilot/v2/CLAIM_BOUNDARIES_V2.md`. Its label-provenance wording was corrected on 2026-09-13 (`2158fe8`) to describe v2.2 labels as the exact consensus of two independent blinded **model** annotators, not oracle/human labels. That edit was documentation-only: no prohibition was relaxed, added, or removed, and no result, method, sample, or threshold changed.

# Current Stage

**Submission stage. The final scientific pilot is COMPLETE and VALID, the Russian APA 7 paper is finalized against it, and the РКНП submission package is built.** The frozen v2.2 protocol was executed end-to-end under `max_tokens = 32768`, `reasoning_effort = low`, 4 workers, Session G then Session P: **600/600 registered calls completed**, zero provider failures, one parser failure (arm B, handled under the preregistered `count_as_failure` policy), all cross-session invariants `PASS`, no VOID marker on either session. The `reasoning_effort = low` amendment resolved the reasoning-token exhaustion that had voided every prior attempt — zero length truncations across all 600 calls.
...
- **РКНП submission package (current)** — `paper/RKNP_SUBMISSION_FINAL_RU.docx`, `.pdf`, `paper/RKNP_SUBMISSION_FINAL_SOURCE.md`, `paper/RKNP_RESEARCH_DIARY_RU.{md,docx}`, `paper/RKNP_SUPERVISOR_REVIEW_DRAFT_RU.{md,docx}`, `paper/RKNP_COMPLIANCE_AUDIT.md`, six figures under `paper/figures/rknp_final/`. Built reproducibly from `paper/rknp_src/` by `scripts/build_rknp_figures.py`, `scripts/build_rknp_submission.py` and `scripts/build_rknp_companions.py`. **No scientific value was recomputed or changed**; every number is carried from the frozen v2.2 artifacts and was re-verified against them (including an independent recomputation of the per-class and per-arm tables from `program_aggregates`). Section page budgets (introduction ≤ 2, research ≤ 20, conclusion ≤ 1) are verified on the actually rendered PDF, not estimated: 2 / 19 / 1. Citations are sequential `[n]` by first appearance, generated in the same pass as the reference list, so an orphan or missing reference is impossible by construction.
...
1. ~~Update `pilot/v2/CLAIM_BOUNDARIES_V2.md`~~ — done in `2158fe8` (documentation-only wording correction; no claim relaxed).
2. ~~Verify the 10 flagged reference entries~~ — done in `2158fe8`; the verified metadata is carried into `paper/rknp_src/references.json`.
3. **Student action (blocking for submission):** supply the author's exact name spelling, class, the official РКНП direction and section, and the supervisor's name. These are the only placeholders left in the package and they appear on the title page only — see `paper/RKNP_COMPLIANCE_AUDIT.md` §"Действия человека".
