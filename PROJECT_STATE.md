...
- **Dataset v2.1** — corrected sample frozen at `selection_frozen_at 2026-09-10T17:51:43Z`; **n = 30**, 30 unique programs, 30 unique `task_id`s; manifest SHA-256 `d53bcdc232f7…`; mechanical sanity `PASS` (`data/manifests/GATE7_DATASET_SANITY.json`). **The manifest file itself remains unmodified and its freeze is not being unilaterally broken** — but its `pattern_label`/`pattern_source` fields are under active, investigated dispute; see Blocked, below, and `data/PATTERN_LABEL_PROVENANCE_AUDIT.md`.
...
- Dataset: ConDefects provenance, mechanical inventory, blinded fault-location ground truth frozen **before** pattern labeling, sampling audits, and the corrected v2.1 sample (Antigravity). **Caveat added after investigation:** fault-location blinding (S1–S4) is confirmed mechanical and compliant; **pattern labeling itself (S6) was not performed by a human annotator as the protocol requires** — see Blocked, below.
...
- **Pattern-label provenance is investigated and found to violate the frozen annotation protocol — this now blocks the next execution attempt, independent of the token-exhaustion blocker below.** Full investigation: `data/PATTERN_LABEL_PROVENANCE_AUDIT.md`. Finding: `pattern_label` for all 30 (and all 232 frame) programs in both the v2.0 and v2.1 samples was assigned entirely by a hardcoded regex/keyword heuristic (`classify_program()`, byte-identical in `scripts/package_final_sample.py` and `scripts/package_final_sample_v2_1.py`) that never consults AtCoder task metadata (its `task_id` parameter is unused) and was never reviewed by a human — not the blinded human annotation `pilot/v2/GROUND_TRUTH_PROTOCOL.md` §6 and `pilot/ANNOTATION_GUIDE.md` §2 require. Direct source inspection of six sampled programs found **two of three checked `binary_search` programs misclassified** (`46026874` is a two-pointer trim + palindrome check; `39342455`'s "binary search" signal is an unused `import bisect`), demonstrating this is a real accuracy defect in the frozen sample, not only a process/documentation gap. `pattern_source: "AST rule"` is a hardcoded constant, not a runtime provenance record, and is also technically inaccurate (no `ast` module is used anywhere — the mechanism is regex text matching, not AST analysis). **The manifest has not been modified.** Two remediation options are proposed in the audit (full protocol-compliant re-annotation vs. reframing the pilot as predicted- rather than oracle-label) and require explicit authorization before either the manifest, the packaging scripts, or `pilot/v2/CLAIM_BOUNDARIES_V2.md` are touched.
...
0. **Resolve pattern-label provenance (`data/PATTERN_LABEL_PROVENANCE_AUDIT.md`) before any further execution attempt.** Requires the user to choose between Option A (protocol-compliant human re-annotation of the 232-program frame, likely producing a new versioned manifest, e.g. v2.2) and Option B (reframe the pilot as predicted- rather than oracle-label, with `CLAIM_BOUNDARIES_V2.md` revised accordingly). Neither is applied yet.
...
2. Re-run the clean scientific pilot: Session G and Session P back-to-back under the frozen invariants (`pilot/v2/PILOT_PROTOCOL_V2.md` §3.1). **Do not start this until item 0 is resolved** — `pattern_label` directly selects each program's condition-C prior, so executing against disputed labels would spend the full 600-call budget on a primary comparison whose causal interpretation is already compromised.
...
- **Pattern-label provenance** — `data/PATTERN_LABEL_PROVENANCE_AUDIT.md`: labels were produced by an unreviewed automated heuristic, not the blinded human annotation the protocol requires; a spot-check found 2 of 3 checked `binary_search` programs misclassified. Supersedes the severity (but not the substance) of V-1 below for the pattern-labeling step specifically.
